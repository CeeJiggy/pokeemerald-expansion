...
#define GROWTH_DURATION(g3, g4, g5, xy, oras, g7) OW_BERRY_GROWTH_RATE == GEN_3 ? g3 : OW_BERRY_GROWTH_RATE == GEN_4    ? g4   \
                                                                                   : OW_BERRY_GROWTH_RATE == GEN_5      ? g5   \
                                                                                   : OW_BERRY_GROWTH_RATE == GEN_6_XY   ? xy   \
                                                                                   : OW_BERRY_GROWTH_RATE == GEN_6_ORAS ? oras \
                                                                                                                        : g7
#define YIELD_RATE(g3, g4, xy, oras) OW_BERRY_YIELD_RATE == GEN_3 ? g3 : OW_BERRY_YIELD_RATE == GEN_4  ? g4 \
                                                                     : OW_BERRY_YIELD_RATE == GEN_6_XY ? xy \
                                                                                                       : oras
            ...
            else
            {
                ...
                while (time > 0 && tree->stage != BERRY_STAGE_BERRIES)
                {
                    ...
                    if (tree->moistureClock % 60 == 0)
                    {
                        if (OW_BERRY_MOISTURE)
                        {
                            drainVal = (OW_BERRY_DRAIN_RATE == GEN_4) ? GetDrainRateByBerryType(tree->berry) : (OW_BERRY_DRAIN_RATE == GEN_6_XY) ? 4
                                                                                                                                                 : 25;
...
static u8 GetMutationOutcome(u8 berry1, u8 berry2)
{
    ...
    {
        if ((sBerryMutations[i][0] == berry1 && sBerryMutations[i][1] == berry2) || (sBerryMutations[i][0] == berry2 && sBerryMutations[i][1] == berry1))
            return (i + 1);
    }
    return 0;
}
...
static u8 TryForMutation(u8 berryTreeId, u8 berry)
{
    ...
    for (j = 0; j < OBJECT_EVENTS_COUNT; j++)
    {
        if (gObjectEvents[j].active && gObjectEvents[j].movementType == MOVEMENT_TYPE_BERRY_TREE_GROWTH && GetStageByBerryTreeId(GetObjectEventBerryTreeId(j)) != BERRY_STAGE_NO_BERRY && j != i)
        {
            ...
            if (Random() % 100 < (OW_BERRY_MUTATION_CHANCE * (mulch == ITEM_TO_MULCH(ITEM_SURPRISE_MULCH) || mulch == ITEM_TO_MULCH(ITEM_AMAZE_MULCH))) && ((x1 == x2 && y1 == y2 - 1) ||
                                                                                                                                                            (x1 == x2 && y1 == y2 + 1) ||
                                                                                                                                                            (x1 == x2 - 1 && y1 == y2) ||
                                                                                                                                                            (x1 == x2 + 1 && y1 == y2)))
