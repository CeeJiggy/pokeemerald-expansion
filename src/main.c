...
void AgbMain()
{
    *(vu16 *)BG_PLTT = RGB_WHITE; // Set the backdrop to white on startup
    ...
    if (DECAP_ENABLED && DECAP_MIRRORING)
        REG_WAITCNT = WAITCNT_PREFETCH_ENABLE | WAITCNT_WS0_S_1 | WAITCNT_WS0_N_3 | WAITCNT_WS1_S_1 | WAITCNT_WS1_N_3 | WAITCNT_WS2_S_1 | WAITCNT_WS2_N_3;
    else
...
#define BCD8(x) ((((x) >> 4) & 0xF) * 10 + ((x) & 0xF))
    ...
    RtcGetInfo(&rtc);
    seconds =
        ((HOURS_PER_DAY * RtcGetDayCount(&rtc) + BCD8(rtc.hour)) * MINUTES_PER_HOUR + BCD8(rtc.minute)) * SECONDS_PER_MINUTE + BCD8(rtc.second);
    SeedRng(seconds);
#undef BCD8
#endif
}
