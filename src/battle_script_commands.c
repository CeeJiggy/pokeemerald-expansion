...
const u16 sLevelCaps[NUM_SOFT_CAPS] = {15, 19, 24, 29, 31, 33, 42, 46};
...
static void Cmd_switchineffects(void)
{
    ...
    else
    {
        ...
        if ((battlerAbility == ABILITY_INTIMIDATE || battlerAbility == ABILITY_DOWNLOAD) && !IsBattlerAlive(BATTLE_OPPOSITE(battler)) && !IsBattlerAlive(BATTLE_PARTNER(BATTLE_OPPOSITE(battler))))
        {
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    if (gSaveBlock2Ptr->optionsLevelCaps)
    {
