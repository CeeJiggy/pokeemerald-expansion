    ...
    do
    {
        ...
        case MOVEEND_DANCER: // Special case because it's so annoying
            if (gMovesInfo[gCurrentMove].danceMove)
            {
                ...
                if (!(gMoveResultFlags & (MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE) || (!gSpecialStatuses[gBattlerAttacker].dancerUsedMove && gBattleStruct->bouncedMoveIsUsed)))
                { // Dance move succeeds
...
void BS_SetPledge(void)
{
    ...
    else if ((gChosenActionByBattler[partner] == B_ACTION_USE_MOVE) && gBattleTypeFlags & BATTLE_TYPE_DOUBLE && IsBattlerAlive(partner) && gCurrentMove != partnerMove && gMovesInfo[partnerMove].effect == EFFECT_PLEDGE)
    {
