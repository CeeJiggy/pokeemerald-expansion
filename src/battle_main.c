...
static void DoBattleIntro(void)
{
    ...
    case 13: // second opponent's mon send out
        ...
        if (B_FAST_INTRO == TRUE && !(gBattleTypeFlags & (BATTLE_TYPE_RECORDED | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_RECORDED_IS_MASTER | BATTLE_TYPE_LINK)))
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    attackerAbility = GetBattlerAbility(battlerAtk);
    if (gMovesInfo[move].type == TYPE_NORMAL && TrySetAteType(move, battlerAtk, attackerAbility) && GetActiveGimmick(battlerAtk) != GIMMICK_DYNAMAX)
    {
        gBattleStruct->ateBoost[battlerAtk] = 1;
    }
    else if (gMovesInfo[move].type != TYPE_NORMAL && gMovesInfo[move].effect != EFFECT_HIDDEN_POWER && gMovesInfo[move].effect != EFFECT_WEATHER_BALL && attackerAbility == ABILITY_NORMALIZE && GetActiveGimmick(battlerAtk) != GIMMICK_Z_MOVE)
    {
        gBattleStruct->dynamicMoveType = TYPE_NORMAL | F_DYNAMIC_TYPE_SET;
