...
// Allow for illegal abilities within tests.
    ...
    case FIRST_TURN_EVENTS_STARTING_STATUS:
        if (!gBattleStruct->startingStatusDone && gBattleStruct->startingStatus && AbilityBattleEffects(ABILITYEFFECT_SWITCH_IN_STATUSES, 0, 0, ABILITYEFFECT_SWITCH_IN_STATUSES, 0) != 0)
        {
...
static void TryChangingTurnOrderEffects(u32 battler1, u32 battler2)
{
    ...
    if (!gProtectStructs[battler1].quickDraw && ((holdEffectBattler1 == HOLD_EFFECT_QUICK_CLAW && gBattleStruct->quickClawRandom[battler1]) || (holdEffectBattler1 == HOLD_EFFECT_CUSTAP_BERRY && HasEnoughHpToEatBerry(battler1, 4, gBattleMons[battler1].item))))
    ...
    // Quick Claw and Custap Berry
    if (!gProtectStructs[battler2].quickDraw && ((holdEffectBattler2 == HOLD_EFFECT_QUICK_CLAW && gBattleStruct->quickClawRandom[battler2]) || (holdEffectBattler2 == HOLD_EFFECT_CUSTAP_BERRY && HasEnoughHpToEatBerry(battler2, 4, gBattleMons[battler2].item))))
...
static void FreeResetData_ReturnToOvOrDoEvolutions(void)
{
    if (!gPaletteFade.active)
    {
        ...
        gDexnavBattle = FALSE;
        ResetSpriteData();
        if (!(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_SAFARI | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_WALLY_TUTORIAL)) && (B_EVOLUTION_AFTER_WHITEOUT >= GEN_6 || gBattleOutcome == B_OUTCOME_WON || gBattleOutcome == B_OUTCOME_CAUGHT))
        {
...
u8 GetTypeBeforeUsingMove(u32 move, u8 battlerAtk)
{
    ...
    else if (gMovesInfo[move].effect == EFFECT_RAGING_BULL && (gBattleMons[battlerAtk].species == SPECIES_TAUROS_PALDEA || gBattleMons[battlerAtk].species == SPECIES_TAUROS_PALDEA_BLAZE || gBattleMons[battlerAtk].species == SPECIES_TAUROS_PALDEA_AQUA))
    {
        return gBattleMons[battlerAtk].types[1];
    }
    else if (gMovesInfo[move].effect == EFFECT_IVY_CUDGEL && (gBattleMons[battlerAtk].species == SPECIES_OGERPON_WELLSPRING || gBattleMons[battlerAtk].species == SPECIES_OGERPON_WELLSPRING_TERA || gBattleMons[battlerAtk].species == SPECIES_OGERPON_HEARTHFLAME || gBattleMons[battlerAtk].species == SPECIES_OGERPON_HEARTHFLAME_TERA || gBattleMons[battlerAtk].species == SPECIES_OGERPON_CORNERSTONE || gBattleMons[battlerAtk].species == SPECIES_OGERPON_CORNERSTONE_TERA))
    {
        return gBattleMons[battlerAtk].types[1];
    }
...
u8 GetMonMoveType(u16 move, struct Pokemon *mon, u32 battler)
{
    ...
    {
        if (moveType != TYPE_NORMAL && gMovesInfo[move].effect != EFFECT_HIDDEN_POWER && gMovesInfo[move].effect != EFFECT_WEATHER_BALL)
            return TYPE_NORMAL;
    }
