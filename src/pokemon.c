...
#define IS_LEAGUE_BATTLE(trainerClass) \
    ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) && (trainerClass == TRAINER_CLASS_ELITE_FOUR || trainerClass == TRAINER_CLASS_LEADER || trainerClass == TRAINER_CLASS_CHAMPION))
...
void AdjustFriendship(struct Pokemon *mon, u8 event)
{
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if ((event != FRIENDSHIP_EVENT_WALKING || !(Random() & 1)) && (event != FRIENDSHIP_EVENT_LEAGUE_BATTLE || IS_LEAGUE_BATTLE(opponentTrainerClass)))
        {
